...
{
    public class SignerReminderServiceTest
    {
        ...
        public void Setup()
        {
            ...
            _mockSigerOptions.SetupGet(x => x.Value).Returns(new SignerOptions() { BasePath = "/signer/", ReminderIntegrationPath = "ReminderIntegration" });
        }

        [Test]
        public void Ensure()
        {
            SignerReminderService signerReminderService = new(_mockHttpClient.Object, _mockSuiteOptions.Object, _mockSigerOptions.Object);

            Assert.IsNotNull(signerReminderService);
            Assert.IsInstanceOf(typeof(SignerServiceBase), signerReminderService);
        }
    }
}
